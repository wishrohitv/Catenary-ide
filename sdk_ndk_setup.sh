...
ANDROID_NDK_ROOT="${ANDROID_SDK_ROOT}/ndk/25b"
...
wget -q "${NDK_URL}" -O "${ANDROID_SDK_ROOT}/ndk-r25b.zip"
unzip -q -d "${ANDROID_SDK_ROOT}" "${ANDROID_SDK_ROOT}/ndk-r25b.zip"
rm "${ANDROID_SDK_ROOT}/ndk-r25b.zip"
mv "${ANDROID_SDK_ROOT}/android-ndk-r25b" "${ANDROID_NDK_ROOT}"
...
cd ~/Android/Sdk/ndk/25b/android-ndk-r25b/
cp -r * ../
cd ~/Android/Sdk
mkdir tools
